...
namespace Dystopia
{
	...
	void SceneView::Init()
	{
		...
		bb = Factory::CreateBox("Another box");
		mpBoxObject2 = GetCurrentScene()->InsertGameObject(Utility::Move(*bb));
		mpBoxObject2->GetComponent<Transform>()->SetScale(Math::Vec4{ 128.f, 128.f, 1.f });
		mpBoxObject2->GetComponent<Transform>()->SetGlobalPosition({ -200.0F, -100.0f, 1 });
		mpBoxObject2->GetComponent<Renderer>()->SetTexture(_bbt);
		mpBoxObject2->GetComponent<Collider>()->Init();
		mpBoxObject2->GetComponent<RigidBody>()->SetRestitution(1.0F);
		mpBoxObject2->GetComponent<RigidBody>()->SetStaticFriction(0.4F);
		mpBoxObject2->GetComponent<RigidBody>()->SetKineticFriction(0.6F);
		mpBoxObject2->GetComponent<RigidBody>()->Init();
		delete bb;
		...
		mpStaticBoxObject->GetComponent<RigidBody>()->SetStaticFriction(0.0F);
		mpStaticBoxObject->GetComponent<RigidBody>()->SetKineticFriction(0.0);
		...
		mpStaticBoxObject = GetCurrentScene()->InsertGameObject(Utility::Move(*s));
		mpStaticBoxObject->GetComponent<RigidBody>()->SetRestitution(0.5F);
		mpStaticBoxObject->GetComponent<RigidBody>()->SetStaticFriction(0.0);
		mpStaticBoxObject->GetComponent<RigidBody>()->SetKineticFriction(0.0);
	...
	void SceneView::Update(const float& _dt)
	{
		...
		if (GetMainEditor().CurrentState() == EDITOR_PLAY)
		{
			...
			{
				mpBoxObject->GetComponent<RigidBody>()->AddForce({ -15,0,0 });
			}
			...
			{
				mpBoxObject->GetComponent<RigidBody>()->AddForce({ 0,25,0 });
			}
		}
